...
int main(){
   ...
   int numCidades,tamanhoPopulacao,geracoes,taxaMutacao,verticeInicial;
   double x,y,peso;
   scanf("%d\n", &numCidades);
   // coord cidades[numCidades];
   Grafo* grafo = new Grafo(numCidades,false);
  ...
  for(int i = 0; i < numCidades; i++){
    for(int j = 0; j < numCidades; j++){
      peso = distancia(cidades[i].x, cidades[j].x, cidades[i].y, cidades[j].y);
      grafo -> inserirAresta(i, j, peso);
    }
  }
   algoritmoGenetico ag(grafo,40,100,20,0);
